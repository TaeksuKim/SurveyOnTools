...
>
> **v2.1 변경:** 확장 지표에 **Headless/CI-Native(#23)·세션 핸드오프(#24)** 추가(Gemini가
> 제기한 CI/CD·Handoff 관점 반영), **Codex Starlark execpolicy 주장을 교차검증 후 사실로
> 확인하여 정정**(§2.5·§5), 관련 1차 출처 보강.
...
| 21 | **Git/Worktree 격리** | ● | ◐ | ◐ | ◐ | ◐ | ◐ |
| 22 | **팀/공유 메모리** (협업 자산화) | ◐ | ○ | ◐ | ◐ | ○ | ◐ |
| 23 | **Headless/CI-Native** (CI/CD·무인 파이프라인) | ● | ● | ◐ | ◐ | ◐ | ◐ |
| 24 | **세션 핸드오프** (cross-device 연속성) | ◐ | ◐ | ◐ | ◐ | ○ | ● |
...
- **Codex CLI (●):** Rust `execpolicy` 크레이트가 **Starlark 정책**(`default.policy`)으로 쉘
  명령을 allow/prompt/forbidden 평가하고, OS 격리는 **macOS Seatbelt + Linux Landlock**을
  사용(전체 컨테이너보다 경량). 기본 네트워크 차단·워크스페이스 한정 쓰기, 클라우드는 격리
  컨테이너. 3축 제어(approval_policy·sandbox_mode·network_access), `/mode`로 세션 중 전환.
  [17][18][24]
...
- **#22 팀/공유 메모리:** 전반적 약점 — Codex는 *명시적으로 팀 공유·cross-machine 동기화
  ...
  → **공통 공백이자 §4의 기회 영역.**
- **#23 Headless/CI-Native:** Claude Code `-p`(print) 헤드리스 + 공식
  `anthropics/claude-code-action` GitHub Action, 출력 text/json/stream-json(●) [25]; Codex
  `codex exec` 단발 스크립트(exit code·cron·CI)(●) [16]; Antigravity GitHub Actions 통합·
  oh-my-openagent CLI 스크립팅·OpenClaw cron·Hermes 게이트웨이 무인 실행(◐). [19][5][9][22]
- **#24 세션 핸드오프:** Hermes는 *텔레그램에서 시작→터미널에서 이어받기* 등 채널 간
  연속성이 1급(●) [10][22]; Claude Code resume/fork·Codex 앱·클라우드 연속은 부분 이전(◐),
  단 Codex 메모리는 cross-machine 동기화 미지원 [8]; oh-my-openagent ○.
...
11. **CI-Native 헤드리스 표준.** Claude Code `-p`+`claude-code-action`, Codex `codex exec`처럼
    동일 에이전트가 대화형/무인(CI) 양쪽에서 동작하고 **구조화 출력(json/stream-json)**과 exit
    code를 내도록 설계 — 파이프라인 통합과 자동화의 핵심. [25][16]
12. **채널 간 세션 핸드오프.** Hermes처럼 한 채널에서 시작한 작업을 다른 채널/기기에서
    중단 없이 이어받는 연속성은 장기 실행 에이전트의 UX 차별점 — 단, 권한·샌드박스 동반 강화. [22]
...
4. **Codex 권한 엔진 — 교차검증 후 정정(상호 보완 사례).** 초판(v2)에서는 Gemini의 "Starlark
   정책 엔진" 주장에 1차 출처 부족을 이유로 유보적이었으나, 지적을 받아 재검증한 결과
   **사실로 확인했다.** Codex는 Rust `execpolicy` 크레이트가 Starlark(`default.policy`)로 명령을
   평가하고 macOS Seatbelt·Linux Landlock으로 격리한다 — 이를 §2.5에 반영했다. *이번 건은 타
   Agent의 지적이 본 리포트의 정확도를 높인 협업 사례다.* [17][24]
...
| Headless/CI 통합 | **Claude Code · Codex**(exec) | 상대적 약함 |
| 세션 핸드오프(채널 간) | 부분(resume/fork) | **Hermes**(다채널 연속) |
...
24. *Execpolicy System / README* — openai/codex (GitHub). https://github.com/openai/codex/blob/rust-v0.107.0/codex-rs/execpolicy/README.md
25. *Headless Mode and CI/CD* — Claude Code (SFEIR Institute) / `anthropics/claude-code-action`. https://institute.sfeir.com/en/claude-code/claude-code-headless-mode-and-ci-cd/
